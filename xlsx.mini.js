...
function make_xlsx_lib(XLSX) {
  ...
  function datenum_local(v, date1904) {
    ...
    return (
      (epoch -
        (SSFdnthresh +
          (v.getTimezoneOffset() - SSFbasedate.getTimezoneOffset()) * 60000)) /
      (24 * 60 * 60 * 1000)
    );
  }
...
*/
    ...
    function _write(cfb, options) {
      ...
      for (i = 0; i < L[4] << 2; ++i) {
        ...
        if (_nm.length > 32) {
          console.error(
            "Name " + _nm + " will be truncated to " + _nm.slice(0, 32)
          );
          _nm = _nm.slice(0, 32);
        }
        flen = 2 * (_nm.length + 1);
    ...
    var _deflateRaw = (function _deflateRawIIFE() {
      ...
      function write_huff_fixed(data, out) {
        ...
        while (boff < data.length) {
          var L = /* data.length - boff; */ Math.min(
            0xffff,
            data.length - boff
          );
    ...
    function write_zip(cfb, options) {
      ...
      for (i = 1; i < cfb.FullPaths.length; ++i) {
        ...
        crcs[fcnt] =
          typeof fi.content == "string"
            ? CRC32.bstr(fi.content, 0)
            : CRC32.buf(fi.content, 0);

        var outbuf =
          typeof fi.content == "string" ? s2a(fi.content) : fi.content;
...
*/
    ...
    function parse_mad(file, options) {
      ...
      // $FlowIgnore
      var data = (
        has_buf && Buffer.isBuffer(file) ? file.toString("binary") : a2s(file)
      ).split("\r\n");
      var di = 0,
        row = "";
    ...
    function write_mad(cfb, options) {
      ...
      for (var i = 1; i < cfb.FullPaths.length; ++i) {
        ...
        // $FlowIgnore
        var cstr =
          has_buf && Buffer.isBuffer(ca) ? ca.toString("binary") : a2s(ca);
        ...
        out.push(boundary);
        out.push(
          "Content-Location: " + (opts.root || "file:///C:/SheetJS/") + fp
        );
        out.push(
  ...
  function fuzzydate(s) {
    ...
    if (y < 0 || y > 8099 || s.match(/[^-0-9:,\/\\(\d\s\d)]/)) return n;
    return o;
  }
  ...
  var unescapexml = (function () {
    ...
    function raw_unescapexml(text) {
      ...
      var j = s.indexOf("]]>");
      return (
        raw_unescapexml(s.slice(0, i)) +
        s.slice(i + 9, j) +
        raw_unescapexml(s.slice(j + 3))
      );
    }
  ...
  var _XLSIcv = rgbify([
    ...
    0x000000 /* 0x40 icvForeground ?? */, 0xffffff /* 0x41 icvBackground ?? */,
  ...
  var DBF = (function () {
    ...
    function dbf_to_aoa(buf, opts) {
      ...
      var hend = Math.min(
        d.length,
        ft == 0x02 ? 0x209 : fpos - 10 - (vfp ? 264 : 0)
      );
      var ww = l7 ? 32 : 11;
      ...
      while (nrow-- > 0) {
        ...
        for (C = 0; C != fields.length; ++C) {
          ...
          var s =
            typeof $cptable !== "undefined"
              ? $cptable.utils.decode(current_cp, dd)
              : a2s(dd);
    ...
    function sheet_to_dbf(ws, opts) {
      ...
      for (i = 0; i <= range.e.c - range.s.c; ++i) {
        ...
        colwidths[i] =
          guess == "C" || _guess == "N" ? maxlen : _RLEN[guess] || 0;
        rlen += colwidths[i];
        coltypes[i] = guess;
      }
      ...
      if (dbf_codepage_map[cp] != +o.codepage) {
        if (o.codepage)
          console.error(
            "DBF Unsupported codepage " + current_codepage + ", using 1252"
          );
      ...
      for (i = 0; i < data.length; ++i) {
        ...
        for (j = 0; j < headers.length; ++j) {
          ...
          switch (coltypes[j]) {
            ...
            case "C":
              var _l = rout.l;
              var _s = String(data[i][j] != null ? data[i][j] : "").slice(
                0,
                colwidths[j]
              );
            ...
            case "NN":
              {
                ...
                for (rj = 1; rj < record.length; ++rj)
                  switch (record[rj].charAt(0)) {
                    ...
                    case "E":
                      nn.Ref =
                        ((opts && opts.sheet) || "Sheet1") +
                        "!" +
                        rc_to_a1(record[rj].slice(1));
                      break;
                  }
                  ...
                  case "K":
                    ...
                    if (val.charAt(0) === '"') {
                      ...
                      if (
                        ...
                        val = numdate(
                          wb.Workbook.WBProps.date1904 ? val + 1462 : val
                        );
                        cell_t = "d";
                      }
            ...
            case "F" /* Format */:
              ...
              for (rj = 1; rj < record.length; ++rj)
                switch (record[rj].charAt(0)) {
                  ...
                  case "P":
                    next_cell_format =
                      formats[parseInt(record[rj].slice(1), 10)];
                    break;
                  case "S":
                    break; /* cell style */
                  case "D":
          ...
          switch (cell.t) {
            case "n":
              if (_DIF_XL) {
                ...
                // TODO: should this call SSF_format?
            ...
            case "s":
              p += make_value_str(
                !_DIF_XL || isNaN(+cell.v) ? cell.v : '="' + cell.v + '"'
              );
              break;
            case "d":
              if (!cell.w)
      ...
      switch (opts.type) {
        ...
        case "buffer":
          if (opts.codepage == 65001)
            str = d.toString("utf8"); // TODO: test if buf
  ...
  function write_comments_xml(data) {
    ...
    data.forEach(function (d) {
      ...
      if (tcnt === 0) {
        ...
        o.push(
          '<comment ref="' + d[0] + '" authorId="' + lastauthor + '"><text>'
        );
        var t = "Comment:\n    " + ts[0] + "\n";
      ...
      (carr[1] || []).forEach(function (c, idx) {
        ...
        if (c.a)
          tcopts.personId =
            ...
            "}";
        o.push(
          writextag("threadedComment", writetag("text", c.t || ""), tcopts)
        );
      });
    });
  ...
  function write_ws_xml_autofilter(data, ws, wb, idx) {
    ...
    for (var i = 0; i < names.length; ++i) {
      ...
      name.Ref =
        formula_quote_sheet_name(wb.SheetNames[idx]) + "!" + fix_range(ref);
      break;
    }
    if (i == names.length)
      names.push({
  ...
  function check_wb(wb) {
    ...
    wb.SheetNames.forEach(function (n, i) {
      ...
      var nn =
        formula_quote_sheet_name(n) + "!" + fix_range(ws["!autofilter"].ref);
      if (DN) DN.Ref = nn;
      else
    ...
    for (; _R < rows.length && R < sheetRows; ++_R) {
      ...
      for (_C = C = 0; _C < elts.length; ++_C) {
        ...
        if (
          l &&
          l.charAt(0) != "#" &&
          l.slice(0, 11).toLowerCase() != "javascript:"
        )
          o.l = { Target: l };
        if (dense) {
      ...
      switch ((Rn[3] = Rn[3].replace(/_.*$/, ""))) {
        ...
        case "map": // <style:map> 16.3
          ...
          if (unescapexml(y["condition"]) == "value()>=0")
            ...
            console.error(
              "ODS number format may be incorrect: " + y["condition"]
            );
          break;
        ...
        case "number": // <number:number> 16.29.3
          ...
          if (+y["decimal-places"] - (+y["min-decimal-places"] || 0))
            tNF += fill(
              "0",
              +y["decimal-places"] - (+y["min-decimal-places"] || 0)
            ); // TODO: should this be "#" ?
          NF += tNF;
          break;
      ...
      switch ((Rn[3] = Rn[3].replace(/_.*$/, ""))) {
          ...
          if (Rn[0].charAt(Rn[0].length - 2) === "/") {
            ...
            switch (q.t) {
              case "boolean":
                q.t = "b";
                q.v =
                  parsexmlbool(ctag["boolean-value"]) ||
                  +ctag["boolean-value"] >= 1;
                break;
              case "float":
                q.t = "n";
        ...
        case "time-style": // 16.27.18 <number:time-style>
          if (Rn[1] === "/") {
            ...
            parse_ods_styles(
              str.slice(nfidx, xlmlregex.lastIndex),
              _opts,
              number_format_map
            );
  ...
  function write_number_format_ods(nf, nfidx) {
    ...
    j: {
      ...
      if (nf.indexOf(";") > -1) {
        console.error(
          "Unsupported ODS Style Map exported.  Using first branch of " + nf
        );
      ...
      if (["y", "m", "d"].indexOf(nf[0]) > -1) {
        ...
        k: for (; i < nf.length; ++i)
          switch ((c = nf[i].toLowerCase())) {
            ...
            default:
              console.error(
                "unrecognized character " + c + " in ODF format " + nf
              );
          }
        if (!has_time) break j;
        nf = nf.slice(i + 1);
      ...
      if (nf[i] == '"') {
        ...
        --i;
        payload +=
          "<number:text>" + escapexml(c.replace(/""/g, '"')) + "</number:text>";
      }
    }
        ...
        .map(function (name) {
          ...
          return (
            ...
            writextag("table:named-range", null, {
              ...
              "table:base-cell-address": odsref.replace(
                /[\.]?[^\.]*$/,
                ".$A$1"
              ),
            })
        ...
        keys(sheetRels[sheet]).forEach(function (n) {
          ...
          if (sheetRels[sheet][n].Type == RELS.TCMNT) {
            ...
            tcomments = tcomments.concat(
              parse_tcmnt_xml(getzipdata(zip, dfile, true), opts)
            );
          }
        });
    ...
    switch (o.bookType || "xlsb") {
      ...
      case "sylk":
        return write_string_type(
          SYLK.from_sheet(wb.Sheets[wb.SheetNames[idx]], o, wb),
          o
        );
      case "htm":
      case "html":
      ...
      case "rtf":
        return write_string_type(
          sheet_to_rtf(wb.Sheets[wb.SheetNames[idx]], o),
          o
        );
      case "eth":
        return write_string_type(
      ...
      for (var C = r.s.c; C <= r.e.c; ++C) {
        ...
        if (hdr[C] != null) {
          if (v == null) {
            ...
            row[hdr[C]] =
              raw &&
              (val.t !== "n" || (val.t === "n" && o.rawNumbers !== false))
                ? v
                : format_cell(val, v, o);
          }
      ...
      keys(JS).forEach(function (k) {
        ...
        if (v && typeof v === "object" && !(v instanceof Date)) {
          ...
          if (typeof v == "number") t = "n";
            ...
            z =
              cell != null && cell.z && fmt_is_date(cell.z)
                ? cell.z
  ...
  function ws_get_cell_stub(ws, R, C) {
    ...
    if (typeof R == "string") {
      if (ws["!data"] != null) {
        ...
        return (
          ws["!data"][RC.r][RC.c] || (ws["!data"][RC.r][RC.c] = { t: "z" })
        );
      }
